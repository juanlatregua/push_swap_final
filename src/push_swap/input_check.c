...
/*   Updated: 2024/09/12 20:45:13 by jsilva-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*sub_arg_process si encuentra una cadena divide en elementos individuales y en nodos
de la pila*/
static t_stack	*ft_sub_arg_process(char **argv)
{
	t_stack	*stack_a;
	...
	while (tmp[i])
	{
		...
		stack_add_bottom(&stack_a, stack_new(value));
		i++;
	}
	ft_memfree_all(tmp);
	return (stack_a);
